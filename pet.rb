class Pet
  def initialize(name, type)
    ...
    puts 'Type the word <help> to watch full list of commands.'
    menu
  end
  ...
  def cycle
    if @age > 20
      puts "Your #{@type} #{@name} became a very old and died."
      close
    end
    if @mood > 25
      puts "Your #{@type} #{@name} became a crazy."
      close
    end
    if @hungry <= 0
      puts "Your #{@type} #{@name} became a " \
        'very hungry and died.'
      close
    end
    if @asleep <= 0
      puts "Your #{@type} #{@name} became a " \
    ...
    if @clean <= 0
      puts "Your #{@type} #{@name} became a " \
        'very unclean and beetles ate it.'
      close
    end
    if @walk <= 0
      puts "Your #{@type} #{@name} became a " \
        'very sad and run away from you.'
      close
    end
    if @energy <= 0
      puts "Your #{@type} #{@name} had a " \
        'nervous breakdown and died in the hospital.'
      close
    end
    @age += 1
    @mood -= rand(3)
